...
class PointDecayThread(thrd.Thread):
    ...
    def __init__(self):
        ...
        log.info("Decay interval: %d (%d)" % (cfg.sysconf["DECAY_INTERVAL"], cfg.sysconf["DECAY_AMOUNT"]))

    def run(self):

        while True:
            time.sleep(cfg.sysconf["DECAY_INTERVAL"])

            try:
                self.dp.decay_points(cfg.sysconf["DECAY_AMOUNT"])
